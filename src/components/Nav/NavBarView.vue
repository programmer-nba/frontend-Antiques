<template>
      ...
      <div class="relative flex h-16 items-center justify-between">
        ...
        <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
          <div class="flex-shrink-0 flex items-center">
            <img src="../../assets/images/logo.png" alt="logo1" @click="$router.push('/home')"
              class="block h-12 m-2 w-auto bg-green-100 rounded-lg hover:bg-green-300" />
          </div>
          <div class="hidden sm:ml-6 sm:flex sm:items-center">
            <div class="flex space-x-4">
              <a v-for="item in navigation" :key="item.name" @click="$router.push(item.href)" :class="[
          ...
          <div class="ml-3 relative">
            <div>
              <button type="button"
                ...
                id="user-menu" aria-haspopup="true" @click="userMenuOpen = !userMenuOpen">
  ...
  ]" :aria-current="item.href === currentPage ? 'page' : undefined">{{ item.name }}</a>
      </div>
    </DisclosurePanel>
  </Disclosure>
</template>
  ...
  data() {
    return {
      navigation: [
        ...
        { name: "พนักงาน", href: "/employee", current: false },
        { name: "สมาชิก", href: "/customer", current: false },
        { name: "จัดการสินค้า", href: "/product", current: false },
